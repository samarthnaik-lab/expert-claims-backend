    ...
    try {
      ...
      if (customer) {
        ...
        if (customer.customer_id) {
          ...
          // Generate email from firstname and lastname if not provided
          let customerEmail = customer.email;
          if (!customerEmail || customerEmail.trim() === '') {
            // Generate email: firstname.lastname@noreply.expertclaims.com
            const firstName = customer.firstName.trim().toLowerCase().replace(/[^a-z0-9]/g, '');
            const lastName = customer.lastName.trim().toLowerCase().replace(/[^a-z0-9]/g, '');
            customerEmail = `${firstName}.${lastName}@noreply.expertclaims.com`;
            console.log(`[Partner] Generated email for customer: ${customerEmail}`);
          }
          ...
          if (existingCustomer) {
            ...
            if (customerEmail) {
              const { data: userByEmail } = await UserModel.findByEmail(customerEmail);
              if (userByEmail) {
            ...
            if (!existingUser) {
              ...
              const username = customerEmail 
                ...
                : (customer.mobileNumber ? `customer_${customer.mobileNumber}` : `customer_${Date.now()}`);
            ...
            // Step 3: Create customer record with user_id
      ...
      if (payments && Array.isArray(payments) && payments.length > 0) {
        console.log('[Partner] Creating payment phases. Count:', payments.length);
        console.log('[Partner] Payment phases input:', JSON.stringify(payments, null, 2));
        ...
        const paymentsData = payments.map((payment, index) => {
          // Parse amounts
          const phaseAmount = payment.phase_amount ? parseInt(payment.phase_amount) : (payment.amount ? parseInt(payment.amount) : 0);
          const paidAmount = payment.paid_amount ? parseInt(payment.paid_amount) : 0;
          
          // Determine status - if status is 'paid' but payment_date is null, change to 'pending'
          // The chk_payment_completion constraint requires payment_date when status is 'paid'
          let paymentStatus = payment.status || 'pending';
          const hasPaymentDate = payment.payment_date && payment.payment_date !== null && payment.payment_date !== 'null';
          
          // If status is 'paid' but no payment_date, change to 'pending' to avoid constraint violation
          if (paymentStatus === 'paid' && !hasPaymentDate) {
            console.warn(`[Partner] Payment phase ${index} has status 'paid' but no payment_date. Changing to 'pending'.`);
            paymentStatus = 'pending';
          }
          
          // If status is 'paid', ensure paid_amount matches phase_amount (or set it)
          if (paymentStatus === 'paid' && paidAmount !== phaseAmount) {
            console.warn(`[Partner] Payment phase ${index} status is 'paid' but paid_amount (${paidAmount}) doesn't match phase_amount (${phaseAmount}). Setting paid_amount to phase_amount.`);
          }
          
          // Build payment data - ensure all required fields are set
          const paymentData = {
            // case_phase_id is auto-generated by database - don't include it
            case_id: caseId, // Required
            phase_name: payment.phase_name || payment.name || `Phase ${index + 1}`, // Required - provide default if missing
            case_type_id: caseTypeId, // Foreign key to case_types
            phase_amount: phaseAmount,
            status: paymentStatus,
            paid_amount: paymentStatus === 'paid' ? phaseAmount : paidAmount, // If paid, ensure paid_amount matches phase_amount
            created_by: userId || payment.created_by || 1, // Use userId from context, fallback to payment.created_by, then system user
            created_time: new Date().toISOString()
          };

          // Validate required fields
          if (!paymentData.phase_name) {
            console.warn(`[Partner] Payment phase ${index} missing phase_name, using default`);
          }
          if (!paymentData.case_id) {
            console.error(`[Partner] Payment phase ${index} missing case_id!`);
          }
          if (!paymentData.case_type_id) {
            console.warn(`[Partner] Payment phase ${index} missing case_type_id (caseType: ${caseType})`);
          }
          ...
          // Only add payment_date if it exists and status is 'paid' (required by constraint)
          if (hasPaymentDate) {
            paymentData.payment_date = payment.payment_date;
          } else if (paymentStatus === 'paid') {
            // If status is 'paid', set payment_date to current date if not provided
            paymentData.payment_date = new Date().toISOString().split('T')[0];
            console.log(`[Partner] Payment phase ${index} status is 'paid' but no payment_date provided. Setting to today's date.`);
          }
        ...
        console.log('[Partner] Prepared payment phases data:', JSON.stringify(paymentsData, null, 2));
        ...
        if (paymentError) {
          ...
          console.error('[Partner] Payment error details:', JSON.stringify(paymentError, null, 2));
          console.error('[Partner] Payment data that failed:', JSON.stringify(paymentsData, null, 2));
          
          // Check for specific error types
          let errorMessage = 'Failed to create payment phases';
          if (paymentError.message) {
            if (paymentError.message.includes('foreign key')) {
              if (paymentError.message.includes('case_id')) {
                errorMessage = `Payment phase creation failed: Invalid case_id '${caseId}'. Case may not exist yet.`;
              } else if (paymentError.message.includes('case_type_id')) {
                errorMessage = `Payment phase creation failed: Invalid case_type_id '${caseTypeId}'. Case type does not exist.`;
              } else if (paymentError.message.includes('created_by')) {
                errorMessage = `Payment phase creation failed: Invalid created_by user_id '${userId || payment.created_by || 1}'. User does not exist.`;
              } else {
                errorMessage = `Payment phase creation failed: Foreign key constraint violation - ${paymentError.message}`;
              }
            } else {
              errorMessage = `Payment phase creation failed: ${paymentError.message}`;
            }
          }
          
          // Log error but don't fail the entire request - return warning in response
          console.error('[Partner] Payment phase creation error:', errorMessage);
        } else {
          ...
          if (createdPayments && createdPayments.length > 0) {
            console.log('[Partner] Created payment phase IDs:', createdPayments.map(p => p.case_phase_id).join(', '));
          }
        }
      } else {
        console.log('[Partner] No payment phases to create (payments:', payments, ')');
      }
      ...
      if (ticket_Stage) {
        ...
        if (historyError) {
          ...
          console.log('[Partner] Stage history created successfully');
        }
      }

      // Return response in the expected format
      const responseMessage = customerCreated ? "Customer added sucess" : "Task created successfully";
      
      // Check if payment phases were created successfully
      let paymentPhasesStatus = null;
      if (payments && Array.isArray(payments) && payments.length > 0) {
        ...
        paymentPhasesStatus = {
          ...
          created: checkPayments?.length || 0,
          success: (checkPayments?.length || 0) === payments.length
        };
      }
      ...
      return res.status(200).json([{
        ...
        case_id: caseId,
        payment_phases: paymentPhasesStatus
      }]);
    } catch (error) {
